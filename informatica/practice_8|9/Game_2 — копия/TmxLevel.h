...
#include <string>//подключаем библиотеки string, vector, map, SFML
#include <vector>
#include <map>
...
struct TmxObject
{
    ...
    std::string GetPropertyString(const std::string &propertyName);//прописывавем protected объекты и функции структуры TmxObject
...
{
    sf::Uint8 opacity = 0;
    std::vector<sf::Sprite> tiles;//прописывавем protected объекты структуры TmxLayer
};
...
class TmxLevel
{
  public:
    ...
    std::vector<TmxObject> GetAllObjects(const std::string &name) const;//прописывавем public объекты и функции класса TmxLevel
  ...
  private:
    ...
    int m_tileWidth = 0;
    int m_tileHeight = 0;
    int m_firstTileID = 0;//прописывавем private объекты класса TmxLevel
